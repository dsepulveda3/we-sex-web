 ...
 export default function PaymentButton() {
   ...
   useEffect(() => {
     const createPreference = async () => {
       const response = await fetch("/api/create-preferences-tantra-discount");
    ...
    <Background id="comprar">
          <Container style={{paddingTop: "5rem", paddingBottom: "6rem", textAlign: "center"}}>
            <Text4>Luego de comprar la guía la recibirás por email</Text4>
              ...
              <Amount><span>AR$ 1.000</span>  AR$ 500</Amount>
              <Title style={{marginTop: "5rem"}}>Compra internacionalmente</Title>
              <BotonVioleta className="pagoPH" id="boton-pago" target="_blank" href={`https://payhip.com/b/Ewx6b/${affiliate}`}>
                  Pago en USD
              </BotonVioleta>
              <Amount><span>U$D  5</span>  U$D  2.5</Amount>
          </Container>
      </Background>
  </div>
    );
