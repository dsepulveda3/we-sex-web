...
export default function PaymentButton() {
   ...
   useEffect(() => {
     const createPreference = async () => {
       const response = await fetch("/api/create-preferences-anal-tantra-fabi");
       const data = await response.json();
   ...
   return (
    ...
    <Background id="comprar">
          <Container style={{paddingTop: "5rem", paddingBottom: "6rem", textAlign: "center"}}>
              ...
              <Title style={{marginTop: "5rem"}}>Compra internacionalmente</Title>
              <BotonVioleta className="pagoPH" id="boton-pago" target="_blank" href={`https://payhip.com/b/elOD7${affiliate}`}>
                  Pago en USD
              </BotonVioleta>
